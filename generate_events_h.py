...
# Event_t your_event2;
# Event_t your_event3;
# then, execute this script from the root of your source files(that contains Event_t)
        ...
        for file in files:
            if file.endswith(".c"):
                ...
                with open(file_path, "r") as f:
                    content = f.read()
                    matches = re.findall(r"Event_t\s+(\w+);", content)
...
def write_to_events_file(event_names, script_dir):
    ...
    with open(events_file, "w") as f:
        ...
        for event_name in sorted_event_names:
            line = f"extern Event_t {event_name};\n"
            f.write(line)
        f.write("#endif\n")
